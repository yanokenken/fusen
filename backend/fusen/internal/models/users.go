...
type userR struct {
	...
	Fusens      FusenSlice      `boil:"Fusens" json:"Fusens" toml:"Fusens" yaml:"Fusens"`
	Preferences PreferenceSlice `boil:"Preferences" json:"Preferences" toml:"Preferences" yaml:"Preferences"`
...
func (r *userR) GetBoards() BoardSlice {
	if r == nil {
		return nil
	}
	return r.Boards
...
func (r *userR) GetFusens() FusenSlice {
	if r == nil {
		return nil
	}
	return r.Fusens
...
func (r *userR) GetPreferences() PreferenceSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "name", "email", "password", "account_type", "created_at", "updated_at", "confirmation_token", "slug", "deleted_at"}
	userColumnsWithoutDefault = []string{"name", "email", "password"}
	userColumnsWithDefault    = []string{"id", "account_type", "created_at", "updated_at", "confirmation_token", "slug", "deleted_at"}
...
// Fusens retrieves all the fusen's Fusens with an executor.
func (o *User) Fusens(mods ...qm.QueryMod) fusenQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"fusens\".\"user_id\"=?", o.ID),
	)

	return Fusens(queryMods...)
}

// Preferences retrieves all the preference's Preferences with an executor.
func (o *User) Preferences(mods ...qm.QueryMod) preferenceQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"preferences\".\"user_id\"=?", o.ID),
	)

	return Preferences(queryMods...)
}
...
func (userL) LoadBoards(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`boards`),
		qm.WhereIn(`boards.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadFusens allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadFusens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`fusens`),
		qm.WhereIn(`fusens.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load fusens")
	}

	var resultSlice []*Fusen
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice fusens")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on fusens")
	}
	if err = results.Err(); err != nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Fusens = append(local.R.Fusens, foreign)
				if foreign.R == nil {
					foreign.R = &fusenR{}
				}
				foreign.R.User = local
...
// LoadPreferences allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPreferences(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`preferences`),
		qm.WhereIn(`preferences.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load preferences")
	}

	var resultSlice []*Preference
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice preferences")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on preferences")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for preferences")
	}
	...
	if singular {
		object.R.Preferences = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &preferenceR{}
			}
			foreign.R.User = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Preferences = append(local.R.Preferences, foreign)
				if foreign.R == nil {
					foreign.R = &preferenceR{}
				}
				foreign.R.User = local
...
// AddFusens adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Fusens.
// Sets related.R.User appropriately.
func (o *User) AddFusens(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Fusen) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.Fusens = append(o.R.Fusens, related...)
	}
...
// AddPreferences adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Preferences.
// Sets related.R.User appropriately.
func (o *User) AddPreferences(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Preference) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.Preferences = append(o.R.Preferences, related...)
	}
