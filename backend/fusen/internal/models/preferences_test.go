...
func testPreferencesExists(t *testing.T) {
	...
	e, err := PreferenceExists(ctx, tx, o.ID)
	if err != nil {
...
func testPreferencesFind(t *testing.T) {
	...
	preferenceFound, err := FindPreference(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testPreferenceToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, preferenceDBTypes, false, preferenceColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Preference struct: %s", err)
	}
	...
	slice := PreferenceSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Preference)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testPreferenceToOneSetOpUserUsingUser(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, preferenceDBTypes, false, strmangle.SetComplement(preferencePrimaryKeyColumns, preferenceColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
...
var (
	preferenceDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `Theme`: `character varying`, `DefaultTab`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_                 = bytes.MinRead
)
