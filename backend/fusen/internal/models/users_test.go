...
func testUserToManyFusens(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, fusenDBTypes, false, fusenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, fusenDBTypes, false, fusenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadFusens(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Fusens); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Fusens = nil
	if err = a.L.LoadFusens(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Fusens); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyPreferences(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, preferenceDBTypes, false, preferenceColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, preferenceDBTypes, false, preferenceColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Preferences().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadPreferences(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Preferences); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Preferences = nil
	if err = a.L.LoadPreferences(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Preferences); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpFusens(t *testing.T) {
	var err error
	...
	foreigners := []*Fusen{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, fusenDBTypes, false, strmangle.SetComplement(fusenPrimaryKeyColumns, fusenColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Fusen{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddFusens(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Fusens[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Fusens[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Fusens().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpPreferences(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e Preference

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Preference{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, preferenceDBTypes, false, strmangle.SetComplement(preferencePrimaryKeyColumns, preferenceColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Preference{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddPreferences(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Preferences[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Preferences[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Preferences().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Email`: `character varying`, `Password`: `character varying`, `AccountType`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `ConfirmationToken`: `character varying`, `Slug`: `character varying`, `DeletedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
